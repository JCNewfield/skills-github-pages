...
{
  /// ***********************************************************
  public partial class warehouse : ContentPageBase
  {
...
// Generate a random 6-digit string containing characters '0' to '9'
string randomString = new string(Enumerable.Repeat("0123456789", 6)
      ...
      item.UniqueID = "I_" + this.Ets.Form.UpdateModelValueByFormKey("Model.RFID", item.Attribute01, "RFID");
      ...
      if (result.Success)
      {
        ...
        var _itemID = this.Ets.Api.Util.Db.ExecuteScalar<int>(@"SELECT Value FROM tSequence WHERE Name = 'ItemID'").Return;
        ...
        itemLog.MaterialID = this.Ets.Form.UpdateModelValueByFormKey("Model.Material", itemLog.MaterialID, "Material");
        itemLog.Quantity = 1;
        itemLog.LocationID = this.Ets.Form.UpdateModelValueByFormKey("Model.Location", itemLog.LocationID, "Location");
        itemLog.Notes = "Recived to HBW storage";
        this.Ets.Api.Data.DbItemLog.Save.ValidateIgnoredForInsertAsNew(itemLog);
      }
      else
      {
        return;
