...
{
  /// ***********************************************************
  public partial class WareHouseTestingTestTest : ContentPageBase
  {
    ...
    private void RemoveExpired(object sender, EventArgs e){

    Trace.Write("ItemID: it is something");
    ...
    // Inner joining both Item and Location to retrieve the item in the location id above
    var item = api.Util.Db.ExecuteScalar<int>("SELECT ID FROM tItem WHERE LocationID = 7");

    int itemID = item.Return;
    ...
    //Trace.Write("ItemID: " + itemIDString);
    ...
    protected override bool ContentPage_Final()
    {
      ...
      Trace.Write("This is only for testing");
