...
{
  /// ***********************************************************
  public partial class WarehouseDashboard : ContentPageBase
  {
    ...
    {
      string test2 = this.Ets.Values.GetAsString("Data.Journal.Selected.Notes");

      // Declare 'result' at a higher scope so it's accessible outside the 'if' block
      string result = "";

      if (!string.IsNullOrEmpty(test2))
      {
        // If the "test2" string is not empty, create a new string "result" that combines it with "Notes: "
        result = "Notes: " + test2;
      }

      // Now 'result' is accessible here
      this.Ets.Values["TEST"] = result;
